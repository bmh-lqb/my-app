                    ...
                    <el-row v-for="(item1, index1) in scope.row.children" :key="index1">
                        ...
                        <el-col :span="20">
                            <el-row v-for="(item2, index2) in item1.children" :key="index2">
                                <!-- 二级权限 -->
                                <el-col :span="5">
                                    <el-tag @close="myClose(scope.row.id, item2.id, scope)" closable type="success">{{ item2.authName }}</el-tag>
                                    <i class="el-icon-arrow-right"></i>
                                </el-col>
                                <!-- 三级权限 -->
                                <el-col :span="19">
                                    <el-tag  @close="myClose(scope.row.id, item3.id, scope)" class="mytag3" type="warning" closable
                                        v-for="(item3, index3) in item2.children" :key="index3">{{ item3.authName }}
                                    </el-tag>
                                </el-col>
                            </el-row>
                        </el-col>
...
export default {
    ...
    methods: {
        ...
        /*
            ...
            scope: 当前行的数据源
        */
        async myClose(roleId, rightsId, scope) {
            // 发送请求到服务器
