<template>
    <el-card>
        ...
        <!-- tabs 标签切换 -->
        <el-tabs class="mytabs" tabPosition="left" @tab-click="tabsClick">
            ...
            <el-tab-pane label="商品参数" name="second">
                <div class="mydiv" v-for="(item, index1) in manyparams" :key="index1">
                    ...
                    <div class="myitem">
                        <el-checkbox v-model="check" border v-for="(val, index2) in item.attr_vals.split(',')"
                            :key="index2">{{ val }}</el-checkbox>
                    </div>
                </div>
            </el-tab-pane>
            ...
            <el-tab-pane label="商品图片" name="fourth">
                <!-- 图片上传框 -->
                <!-- file-list：上传文件列表集合 -->
                <el-upload class="upload-demo" :headers="uploadHead"
                    action="http://localhost:8888/api/private/v1/upload" :file-list="fileList" list-type="picture"
                    :on-success="uploadSuccess" :on-remove="remove" :on-preview="preview">
